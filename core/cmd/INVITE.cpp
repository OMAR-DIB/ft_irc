...
void Cmd::handleINVITE(Server &s,Client &client, const std::string &command)
{
	...
	if (tokens.size() < 3)
	{
		s.sendToClient(client.GetFd(), ":server 461 " + client.getNickname() + " INVITE :Not enough parameters\r\n");
		return;
	}
	...
	{
		s.sendToClient(client.GetFd(), ":server 484 " + client.getNickname() + " " + channelName + " :Cannot invite yourself\r\n");
		return;
	}
	...
	{
		s.sendToClient(client.GetFd(), ":server 443 " + client.getNickname() + " " + targetNick + " " + channelName + " :is already on channel\r\n");
		return;
	}
	...
	s.sendToClient(targetClient->GetFd(), inviteMsg);

	// Send confirmation to inviter
	s.sendToClient(client.GetFd(), ":server 341 " + client.getNickname() + " " + targetNick + " " + channelName + "\r\n");
	...
	for (size_t i = 0; i < channelClients.size(); i++)
	{
		...
		{
			s.sendToClient(channelClients[i]->GetFd(), notifyMsg);
		}
	}
