...
void Server::handleKICK(Client &client, const std::string &command)
{
	...
	{
		sendToClient(client.GetFd(), ":server 441 " + client.getNickname() + " " + targetNick + " " + channelName + " :They aren't on that channel\r\n");
		return;
	}
	...
	{
		sendToClient(client.GetFd(), ":server 484 " + client.getNickname() + " " + channelName + " :Cannot kick yourself\r\n");
		return;
	}
	...
	for (size_t i = 0; i < channelClients.size(); i++)
	{
		sendToClient(channelClients[i]->GetFd(), kickMsg);
	}
...
void Server::handleINVITE(Client &client, const std::string &command)
{
	...
	if (targetClient == &client)
	{
		sendToClient(client.GetFd(), ":server 484 " + client.getNickname() + " " + channelName + " :Cannot invite yourself\r\n");
		return;
	}
	...
	if (channel->hasClient(targetClient))
	{
		sendToClient(client.GetFd(), ":server 443 " + client.getNickname() + " " + targetNick + " " + channelName + " :is already on channel\r\n");
		return;
	}
	...
	for (size_t i = 0; i < channelClients.size(); i++)
	{
		...
		{
			sendToClient(channelClients[i]->GetFd(), notifyMsg);
		}
	}
